...
pub struct ElfHeader<'slice, C, E> {
    ...
    /// The [`ClassElfHeaderParse`] of this ELF header.
    pub(crate) class: C,
...
impl<'slice, C: ClassParse, E: EncodingParse> ElfHeader<'slice, C, E> {
    ...
    /// # Errors
    ///
    /// - [`ParseElfHeaderError::UnsupportedClass`] is returned when the ELf class is not
    ///     supported.
    /// - [`ParseElfHeaderError::UnsupportedEncoding`] is returned when the ELF encoding is not
    ///     supported.
    /// - [`ParseElfHeaderError::SliceTooSmall`] is returned when the given `slice` is too small
    ///     to contain an ELF file header.
    pub fn parse(slice: &'slice [u8]) -> Result<Self, ParseElfHeaderError> {
        let ident = ElfIdent::new(slice).ok_or(ParseElfHeaderError::SliceTooSmall)?;
        ...
        if slice.len() < class.expected_elf_header_size() {
            return Err(ParseElfHeaderError::SliceTooSmall);
        }
    ...
    /// Returns the [`ElfIdent`] of this ELF file.
    pub fn identifier(&self) -> ElfIdent<'slice> {
        match ElfIdent::new(self.bytes) {
            Some(elf_ident) => elf_ident,
            None => unreachable!(),
        }
    }
...
/// Various errors that can occur while parsing a [`ElfHeader`].
...
pub enum ParseElfHeaderError {
    ...
    /// The slice is too small to contain an ELF file header.
    SliceTooSmall,
}
...
impl fmt::Display for ParseElfHeaderError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::SliceTooSmall => write!(f, "given slice is too small"),
        }
    }
}
