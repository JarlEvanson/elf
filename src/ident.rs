...
use core::{error, fmt, marker::PhantomData, mem};

use crate::{MinimalParse, ParseState, Raw};
...
impl<'slice> ElfIdent<'slice, Raw> {
    /// Creates a new [`ElfIdent<Raw>`] from the given `slice`, returning `None` if `slice` too
...
impl<'slice> ElfIdent<'slice, MinimalParse> {
    /// Creates a new [`ElfIdent<MinimalParse>`] from the given `slice`.
    ///
    /// # Errors
    /// - [`ParseElfIdentMinimalError::TooSmall`]: Returned when the given `slice` is too small to
    ///     contain an [`ElfIdent`].
    /// - [`ParseElfIdentMinimalError::InvalidMagicBytes`]: Returned when the [`ElfIdent`]'s magic
    ///     bytes are invalid.
    /// - [`ParseElfIdentMinimalError::UnsupportedElfHeaderVersion`]: Returned when the ELF header
    ///     version is not supported.
    /// - [`ParseElfIdentMinimalError::NonZeroPadding`]: Returned when the padding of [`ElfIdent`]
    ///     is non-zero.
    pub fn minimal_parse(slice: &'slice [u8]) -> Result<Self, ParseElfIdentMinimalError> {
        let ident = ElfIdent::new(slice).ok_or(ParseElfIdentMinimalError::TooSmall)?;

        if ident.magic() != Self::MAGIC_BYTES {
            return Err(ParseElfIdentMinimalError::InvalidMagicBytes(ident.magic()));
        }

        if ident.header_version() != Self::CURRENT_HEADER_VERSION {
            return Err(ParseElfIdentMinimalError::UnsupportedElfHeaderVersion(
                ident.header_version(),
            ));
        }

        if ident.padding().into_iter().any(|val| val != 0) {
            return Err(ParseElfIdentMinimalError::NonZeroPadding(ident.padding()));
        }
...
impl<S: ParseState> ElfIdent<'_, S> {
...
impl<S: ParseState> core::fmt::Debug for ElfIdent<'_, S> {
...
/// Various errors that can occur while running [`ElfIdent::minimal_parse()`].
#[derive(Clone, Copy, Debug, Hash, PartialEq, Eq, PartialOrd, Ord)]
pub enum ParseElfIdentMinimalError {
    /// The given `slice` is too small to contain an [`ElfIdent`].
    TooSmall,
...
impl fmt::Display for ParseElfIdentMinimalError {
...
impl error::Error for ParseElfIdentMinimalError {}
