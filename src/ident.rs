...
impl<'slice> ElfIdent<'slice> {
    ...
    /// Creates a new [`ElfIdent`] from the given `slice`, returning `None` if `slice` is less
    /// than 16 bytes.
    ...
    /// Creates a new [`ElfIdent`] from the given `bytes`.
    pub const fn new_exact(bytes: &'slice [u8; 16]) -> Self {
        Self { bytes }
    }
